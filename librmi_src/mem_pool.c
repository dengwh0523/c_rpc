...
mem_create_pool(size_t size)
{
    ...
    size = (size == 0) ? MEM_DEFAULT_POOL_SIZE : size;
    p = (mem_pool *)mem_memalign(MEM_POOL_ALIGNMENT, size);
    if (p == NULL) {
        return NULL;
    }
...
void *
mem_pnalloc(mem_pool *pool, size_t size)
{
    u_char      *m;
    mem_pool  *p;
...
mem_palloc_block(mem_pool *pool, size_t size)
{
    ...
    m = (u_char *)mem_memalign(MEM_POOL_ALIGNMENT, psize);
    if (m == NULL) {
        return NULL;
    }
